...
export async function loader(_: Route.LoaderArgs) {
  try {
    const res = await fetch(
      "https://api.github.com/repos/Reddishye/raisu/releases/latest",
      { headers: { Accept: "application/vnd.github+json" } }
    );
    if (res.ok) {
      const json = await res.json();
      return { latestVersion: json.tag_name as string };
    }
  } catch {}
  return { latestVersion: "v1.0.0" };
}
...
const GITHUB_URL = "https://github.com/Reddishye/raisu";
const GROUP_ID = "com.github.Reddishye.raisu";

type BuildTool = "gradle-kts" | "gradle-groovy" | "maven";
type RaisuModule = "raisu-paper" | "raisu-spigot" | "raisu-core";

const BUILD_TOOLS: { id: BuildTool; label: string; lang: string; display: string }[] = [
  { id: "gradle-kts",    label: "Gradle KTS",    lang: "kotlin",  display: "kotlin"  },
  { id: "gradle-groovy", label: "Gradle Groovy", lang: "groovy",  display: "groovy"  },
  { id: "maven",         label: "Maven",         lang: "markup",  display: "xml"     },
];

const RAISU_MODULES: { id: RaisuModule; desc: string }[] = [
  { id: "raisu-paper",  desc: "Paper (recommended)" },
  { id: "raisu-spigot", desc: "Spigot / Bukkit"     },
  { id: "raisu-core",   desc: "Standalone"           },
];

function getDepSnippet(tool: BuildTool, mod: RaisuModule, version: string): string {
  if (tool === "gradle-kts") {
    return `repositories {
...
dependencies {
    implementation("${GROUP_ID}:${mod}:${version}")
}`;
  }
  if (tool === "gradle-groovy") {
    return `repositories {
...
function DependencySelector({ defaultVersion }: { defaultVersion: string }) {
  const [tool, setTool] = useState<BuildTool>("gradle-kts");
  const [mod, setMod] = useState<RaisuModule>("raisu-paper");
  const [version, setVersion] = useState(defaultVersion);
  const [copied, setCopied] = useState(false);

  const activeTool = BUILD_TOOLS.find((t) => t.id === tool)!;
  const snippet = getDepSnippet(tool, mod, version);
  const grammar = Prism.languages[activeTool.lang];
  const html = grammar ? Prism.highlight(snippet, grammar, activeTool.lang) : snippet;
  ...
  return (
    <div className="rounded-xl border border-zinc-800/60 bg-zinc-900/40 overflow-hidden my-5">
      {/* Tab + selectors bar */}
      <div className="flex items-center gap-2 px-3 py-2 border-b border-zinc-800/60 bg-zinc-900/60 flex-wrap">
        ...
        <select
          value={mod}
          onChange={(e) => setMod(e.target.value as RaisuModule)}
          className="text-xs text-zinc-300 bg-zinc-800/60 border border-zinc-700/50 rounded-md px-2 py-1 focus:outline-none focus:border-violet-500/50 cursor-pointer"
        >
          {RAISU_MODULES.map((m) => (
        ...
        <div className="flex items-center gap-1.5 ml-auto shrink-0">
          <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-semibold">ver</span>
          <input
            type="text"
            value={version}
            onChange={(e) => setVersion(e.target.value)}
            spellCheck={false}
            className="text-xs text-zinc-300 bg-zinc-800/60 border border-zinc-700/50 rounded-md px-2 py-1 w-20 focus:outline-none focus:border-violet-500/50 font-mono"
          />
        </div>
      </div>
      {/* Code header */}
      ...
      {/* Code */}
      <pre className={`p-5 overflow-x-auto text-sm leading-relaxed language-${activeTool.lang}`}>
        <code
          className={`language-${activeTool.lang}`}
          dangerouslySetInnerHTML={{ __html: html }}
        />
      </pre>
    </div>
...
export default function DocsPage() {
  const { latestVersion } = useLoaderData<typeof loader>();
          ...
          <div className="max-w-3xl mx-auto px-6 sm:px-10 py-10 pb-24 space-y-16">
            ...
            <section id="overview" className="scroll-mt-20">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-zinc-700/50 bg-zinc-800/40 text-zinc-400 hover:text-zinc-200 hover:border-zinc-600/60 hover:bg-zinc-800/70 transition-all text-sm"
              >
                <ExternalLink className="w-3.5 h-3.5 shrink-0" />
                <span className="font-mono text-xs">Reddishye/raisu</span>
                <span className="px-1.5 py-0.5 rounded-md bg-violet-500/15 border border-violet-500/20 text-violet-300 text-[10px] font-mono font-semibold">
                  {latestVersion}
                </span>
              </a>
            </section>
            ...
            <section id="gradle" className="scroll-mt-20">
              ...
              <p className="text-zinc-400 text-sm leading-relaxed mb-4">
                ...
                <a
                  href="https://jitpack.io/#Reddishye/raisu"
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                . Select your build tool and module below — the snippet is generated for you.
              </p>
              <DependencySelector defaultVersion={latestVersion} />
              <Callout>
                {ic(
                  "For most Minecraft plugins, depend on `raisu-paper` (Paper servers) or `raisu-spigot` (Spigot/Bukkit). Both transitively include `raisu-core` and `raisu-api`."
                )}
